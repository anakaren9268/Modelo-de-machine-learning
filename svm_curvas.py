...
# -*- coding: utf-8 -*-
"""
Script para generar curvas y métricas del modelo SVM
Autor: Asistente IA
Fecha: 2024
"""
...
def main():
    """
    Función principal que ejecuta todo el análisis del modelo SVM
    """
    print("=" * 70)
    print("ANÁLISIS DE CURVAS Y MÉTRICAS - MODELO SVM")
    print("=" * 70)
    ...
    print("\n[INFO] Entrenando modelo SVM...")
    
    # Configuración del modelo según resultados anteriores
    svm_model = SVC(C=100.0, kernel='linear', gamma=0.001, random_state=42, probability=True)
    
    # Entrenar modelo
    svm_model.fit(X_train, y_train)
    print("[OK] Modelo SVM entrenado con C=100.0, kernel='linear', gamma=0.001")
    
    # === VALIDACIÓN CRUZADA ===
    print("\n[INFO] Realizando validación cruzada...")
    
    # Validación cruzada con k=10
    cv_scores_acc = cross_val_score(svm_model, X_train, y_train, cv=10, scoring='accuracy')
    cv_scores_prec = cross_val_score(svm_model, X_train, y_train, cv=10, scoring='precision_macro')
    cv_scores_rec = cross_val_score(svm_model, X_train, y_train, cv=10, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(svm_model, X_train, y_train, cv=10, scoring='f1_macro')
    ...
    train_sizes, train_scores, val_scores = learning_curve(
        svm_model, X_train, y_train, cv=10, n_jobs=-1,
    ...
    plt.title('Curva de Aprendizaje - SVM', fontsize=16, fontweight='bold', pad=20)
    ...
    # === RESUMEN FINAL ===
    print("\n" + "=" * 70)
    print("RESUMEN FINAL - MODELO SVM")
    print("=" * 70)
    ...
    print(f"   Algoritmo: Support Vector Machine")
    print(f"   C: 100.0")
    ...
    svm_model_medicion = SVC(C=100.0, kernel='linear', gamma=0.001, random_state=42, probability=True)
    
    # Medir costo computacional
    resultados_svm = medir_costo_computacional(
        svm_model_medicion, X_train, y_train, X_val, y_val, "SVM"
    )
    ...
    # Guardar resultados
    guardar_resultados(resultados_df, 'resultados_costos_svm.csv')
