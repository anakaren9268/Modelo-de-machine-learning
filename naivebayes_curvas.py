...
# -*- coding: utf-8 -*-
"""
Script para generar curvas y métricas del modelo Naive Bayes
Autor: Asistente IA
Fecha: 2024
"""
...
def main():
    """
    Función principal que ejecuta todo el análisis del modelo Naive Bayes
    """
    print("=" * 70)
    print("ANÁLISIS DE CURVAS Y MÉTRICAS - MODELO NAIVE BAYES")
    print("=" * 70)
    ...
    print("\n[INFO] Entrenando modelo Naive Bayes...")
    ...
    print("[OK] Modelo Naive Bayes entrenado con var_smoothing=1e-05")
    
    # === VALIDACIÓN CRUZADA ===
    print("\n[INFO] Realizando validación cruzada...")
    
    # Validación cruzada con k=10
    cv_scores_acc = cross_val_score(nb_model, X_train, y_train, cv=10, scoring='accuracy')
    cv_scores_prec = cross_val_score(nb_model, X_train, y_train, cv=10, scoring='precision_macro')
    cv_scores_rec = cross_val_score(nb_model, X_train, y_train, cv=10, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(nb_model, X_train, y_train, cv=10, scoring='f1_macro')
    ...
    train_sizes, train_scores, val_scores = learning_curve(
        nb_model, X_train, y_train, cv=10, n_jobs=-1,
    ...
    plt.title('Curva de Aprendizaje - Naive Bayes', fontsize=16, fontweight='bold', pad=20)
    ...
    # === RESUMEN FINAL ===
    print("\n" + "=" * 70)
    print("RESUMEN FINAL - MODELO NAIVE BAYES")
    print("=" * 70)
    ...
    nb_model_medicion = GaussianNB(var_smoothing=1e-05)
    
    # Medir costo computacional
    resultados_nb = medir_costo_computacional(
        nb_model_medicion, X_train, y_train, X_val, y_val, "Naive Bayes"
    )
    
    # Crear DataFrame con resultados
    resultados_df = pd.DataFrame([resultados_nb])
    
    # Mostrar resumen de costos
    mostrar_resumen_costos(resultados_df)
    
    # Guardar resultados
    guardar_resultados(resultados_df, 'resultados_costos_naivebayes.csv')
