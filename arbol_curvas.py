...
# -*- coding: utf-8 -*-
"""
Script para generar curvas y métricas del modelo Árbol de Decisión
Autor: Asistente IA
Fecha: 2024
"""
...
def main():
    """
    Función principal que ejecuta todo el análisis del modelo Árbol de Decisión
    """
    print("=" * 70)
    print("ANÁLISIS DE CURVAS Y MÉTRICAS - MODELO ÁRBOL DE DECISIÓN")
    print("=" * 70)
    ...
    print("\n[INFO] Entrenando modelo Árbol de Decisión...")
    ...
    print("[OK] Modelo Árbol de Decisión entrenado con max_depth=None, min_samples_split=20, min_samples_leaf=5")
    
    # === VALIDACIÓN CRUZADA ===
    print("\n[INFO] Realizando validación cruzada...")
    
    # Validación cruzada con k=10
    cv_scores_acc = cross_val_score(dt_model, X_train, y_train, cv=10, scoring='accuracy')
    cv_scores_prec = cross_val_score(dt_model, X_train, y_train, cv=10, scoring='precision_macro')
    cv_scores_rec = cross_val_score(dt_model, X_train, y_train, cv=10, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(dt_model, X_train, y_train, cv=10, scoring='f1_macro')
    ...
    y_train_proba = dt_model.predict_proba(X_train)
    ...
    train_sizes, train_scores, val_scores = learning_curve(
        dt_model, X_train, y_train, cv=10, n_jobs=-1,
    ...
    plt.title('Curva de Aprendizaje - Árbol de Decisión', fontsize=16, fontweight='bold', pad=20)
    ...
    # === RESUMEN FINAL ===
    print("\n" + "=" * 70)
    print("RESUMEN FINAL - MODELO ÁRBOL DE DECISIÓN")
    print("=" * 70)
    ...
    print(f"   Algoritmo: Decision Tree Classifier")
    ...
    dt_model_medicion = DecisionTreeClassifier(
        max_depth=None, 
        min_samples_split=20, 
        min_samples_leaf=5, 
        random_state=42
    ...
    resultados_dt = medir_costo_computacional(
        dt_model_medicion, X_train, y_train, X_val, y_val, "Árbol de Decisión"
    )
    
    # Crear DataFrame con resultados
    resultados_df = pd.DataFrame([resultados_dt])
    
    # Mostrar resumen de costos
    mostrar_resumen_costos(resultados_df)
    
    # Guardar resultados
    guardar_resultados(resultados_df, 'resultados_costos_arbol.csv')
